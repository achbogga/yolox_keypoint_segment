...
class COCODataset(Dataset):
    ...
    def __init__(
            self,
            data_dir="/media/aboggaram/primary_storage/data/Octiva/coco_format_keypoints_Jan_19_2023/images",
            json_file="/media/aboggaram/primary_storage/data/Octiva/coco_format_keypoints_Jan_19_2023/train.json",
            name="octiva_kp_train",
            img_size=(416, 416),
            preproc=None,
            cache=False,
            keypoints=1,
    ...
    def annToFontMask(self, img_info, ann):
        h, w = img_info['height'], img_info['width']
        bg = np.zeros((h, w), np.uint8)
        for num in range(len(ann['segmentation'])):
            segm = ann['segmentation'][num]
            if '+' in segm:
                seg_ = segm.split('+')
                for se_ in seg_:
                ...
                if num == 0:
                    ...
                    cv2.fillPoly(bg, np.array(font_array), 0)
        return bg
    ...
    def load_anno_from_ids(self, id_):
        ...
        for obj in annotations:
            ...
            if self.keypoints > 0:
                pts = np.array(obj['keypoints'], np.float32).reshape(-1, 3)
                for i in range(len(pts)):
                    pts[i, 0] = np.min((width, np.max((0, pts[i, 0]))))
        ...
        return (res, img_info, resized_info, file_name, seg_res, img)

    def load_anno(self, index):
        return self.annotations[index][0]

    def load_resized_img(self, index, img):
        if index != -1:
    ...
    def pull_item(self, index):
        if self.random_dataset:
            res, img_info, resized_info, _, seg_res, img = self.load_anno_from_ids(0)
            index = -1
        else:
            res, img_info, resized_info, _, seg_res, img = self.annotations[index]
        id_ = self.ids[index]
        if self.imgs is not None:
            pad_img = self.imgs[index]
            img = pad_img[: resized_info[0], : resized_info[1], :].copy()
        else:
            img = self.load_resized_img(index, img)  # True
        return img, res.copy(), img_info, np.array([id_]), seg_res
    ...
    def __getitem__(self, index):
        ...
        if self.preproc is not None:  # data_augment/TrainTransform
